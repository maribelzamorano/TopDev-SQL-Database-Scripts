 ...
 * $Rev: _ $
 * -------------------------------------------
 * $Workfile: (020) akkradm.pss_GRUP_IUD.sql $
 *
 */

if not exists ( select * from sysobjects where id = object_id('akkradm.pss_GRUP_IUD') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
begin
	exec( N'create procedure akkradm.pss_GRUP_IUD
	as begin
...
alter procedure akkradm.pss_GRUP_IUD
...
as begin
	...
	begin
		...
		if ( @AAKT_CODE in ( 20, 30, 90 )) and 
		   ( not exists( select 1 from akkradm.IO_GRUPPE where GRUP_ID = @GRUP_ID and GRUP_USER = @GRUP_USER and GRUP_#PTS = @GRUP_XPTS ))
		begin
			set @rc = -1;
...
--				end
			...
			begin
				...
				delete from akkradm.IO_GRUPPE where GRUP_ID = @GRUP_ID and GRUP_USER = @GRUP_USER and GRUP_#PTS = @GRUP_XPTS;
