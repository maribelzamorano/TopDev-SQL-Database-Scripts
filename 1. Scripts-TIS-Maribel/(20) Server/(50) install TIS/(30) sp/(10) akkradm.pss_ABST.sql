/*	
 * topdev GmbH, erstellt am 06.10.2009 13:17
 *
 * $Author: Maribel Zamorano $
 * $Date: 2009-10-06 13:17:45 $
 * $Rev: _ $
 * -------------------------------------------
 * $Workfile: (010) akkradm.pss_ABST.sql $
 *
 */

if not exists ( select * from sysobjects where id = object_id('akkradm.pss_ABST') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
begin
	exec( N'create procedure akkradm.pss_ABST
	as begin
...
alter procedure akkradm.pss_ABST
...
as begin
	...
	set @tbsh_shortname = N'ABST';
	set @tbsh_longname = N'ANBIETERSTATUS';
...
--		if ( ltrim( rtrim( @Suchbegriff )) = N'' )	set @Suchbegriff = null;


		if ( @rc = 0 ) and ( @ABST_ID is not null ) and ( isnumeric( @ABST_ID ) = 0 )
		begin
			set @rc = -1;
			set @parm = N'@ABST_ID';
		end
	...
	begin
		...
		if ( @parms > N'' )
				set @parms = @parms + N',';
		set @parms = N'@GEPA_#NAME_12 nvarchar(202), @ABIE_NUMMER nvarchar(25)';  --@Suchbegriff

		if ( @GEPA_#NAME_12 is not null )	-- Sample processing of string values
		begin
			if ( @where > N'' )
		...
		if ( @ABIE_NUMMER is not null )	-- Sample processing of string values
		begin
			if ( @where > N'' )
				set @where = @where + N' and ';
	...
	begin
		...
		if isNull( @orderby, N'' ) = N''
			set @orderby = N'akkradm.V_GESCHAEFTSPARTNER.GEPA_#NAME_12 ';	
	...
	begin
		...
		begin
			...
			set	@sql = 
					...
					N'select T_ANBIETERSTATUS.ABST_ID, ROW_NUMBER() OVER ( ' + @orderby + N' ) as ROWNUMBER from akkradm.T_ANBIETERSTATUS 
					  join T_ANBIETER_STATUS_HIST on T_ANBIETERSTATUS.ABST_ID=T_ANBIETER_STATUS_HIST.ABST_ID
					  join T_ANBIETER on T_ANBIETER_STATUS_HIST.ABIE_ID=T_ANBIETER.ABIE_ID
					  join T_AKKREDITIERUNGSBESCHEID on T_ANBIETER.ABIE_ID=T_AKKREDITIERUNGSBESCHEID.ABIE_ID
	...
	begin
		BEGIN TRY
			...
			set @sql = 
				...
				N'T_ANBIETERSTATUS.ABST_ID as id,
				GEPA_#NAME_12 as NameVoll,
				ABIE_NUMMER as AnbieterNummer,
				ABIE_SCHWERPUNKTE as Schwerpunkte,
				ABST_BEZEICHNUNG as AnbieterstatusDecode,
				ABIE_PTS_ABST as AnbieterstatusZeitpunkt ' +
		
				N'from akkradm.T_ANBIETERSTATUS ' +
				N'join ' + @object_name + N' rs on ( rs.ABST_ID = akkradm.T_ANBIETERSTATUS.ABST_ID ) 
				  join T_ANBIETER_STATUS_HIST on T_ANBIETERSTATUS.ABST_ID=T_ANBIETER_STATUS_HIST.ABST_ID
				  join T_ANBIETER on T_ANBIETER_STATUS_HIST.ABIE_ID=T_ANBIETER.ABIE_ID
				  join T_AKKREDITIERUNGSBESCHEID on T_ANBIETER.ABIE_ID=T_AKKREDITIERUNGSBESCHEID.ABIE_ID
				  join T_AKKREDITEUR on T_AKKREDITIERUNGSBESCHEID.AKRT_ID=T_AKKREDITEUR.AKRT_ID
