 ...
 * $Rev: _ $
 * -------------------------------------------
 * $Workfile: (010) akkradm.pss_GRUP.sql $
 *
 */

if not exists ( select * from sysobjects where id = object_id('akkradm.pss_GRUP') and OBJECTPROPERTY(id, 'IsProcedure') = 1)
begin
	exec( N'create procedure akkradm.pss_GRUP
	as begin
...
alter procedure akkradm.pss_GRUP
...
--> set sortation or using standard sortation
	...
	begin
		...
		if isNull( @orderby, N'' ) = N''
			set @orderby = N'akkradm.T_GRUPPE.GRUP_NAME ';
	...
	begin
		...
		begin
			...
			set	@sql = 
					...
					N'select GRUP_ID, ROW_NUMBER() OVER ( ' + @orderby + N' ) as ROWNUMBER from akkradm.T_GRUPPE ';
	...
	begin
		BEGIN TRY
			...
			set @sql = 
				...
				N'from akkradm.T_GRUPPE ' +
				N'join ' + @object_name + N' rs on ( rs.GRUP_ID = akkradm.T_GRUPPE.GRUP_ID ) ' +
				-- do not change
